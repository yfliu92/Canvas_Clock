...
// const endTime = new Date(2016, 1, 21, 13, 25, 1)
    ...
    for (var j = 0; j < digit[num][i].length; j++) {
      if (digit[num][i][j] == 1) {
        var aBall = {
          ...
          g: 1.5 + Math.random(),
          vx: Math.pow(-1, Math.ceil(Math.random() * 1000)) * 4, //取-4或者+4
          vy: -5,
...
function render(context) {
  ...
  renderDigit(
    ...
    parseInt(hours % 10),
    context
  ) //2*7*(RADIUS+1)为了距离，变成15
  renderDigit(MARGIN_LEFT + 30 * (RADIUS + 1), MARGIN_TOP, 10, context) //冒号是4*10，宽度是4位
  ...
  renderDigit(MARGIN_LEFT + 69 * (RADIUS + 1), MARGIN_TOP, 10, context) //冒号是4*10，宽度是4位
  ...
  for (var i = 0; i < digit[num].length; i++) {
    for (var j = 0; j < digit[num][i].length; j++) {
      if (digit[num][i][j] == 1) {
        cxt.beginPath()
        cxt.arc(
          x + 2 * j * RADIUS + RADIUS + 1,
          y + 2 * i * RADIUS + RADIUS + 1,
          RADIUS,
          0,
          2 * Math.PI
        )
